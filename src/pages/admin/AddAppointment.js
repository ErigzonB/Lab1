            ...
            <div className='container min-vh-100'>
                <div className='min-vh-100 d-flex justify-content-center align-items-center'>
                    <form className='p-5 col-12 col-md-8 col-lg-6 row border bg-white position-relative rounded' >
                        <p style={{ background: "-webkit-linear-gradient(left, #0072ff, #8811c5)", height: "5rem" }} className='text-white text-center position-absolute top-0 start-0 fs-3 d-flex justify-content-center align-items-center'>Cakto Terminin</p>
                        <div className='mt-5 pt-3 col-12 col-md-6 d-flex flex-column align-items-start'>
                            ...
                            <select id='inputDoc' className="form-select">
                                {
                                    doctors.map((doctor, index) => (
                                        <option key={index} value={doctor.Id}>{doctor.name + " " + doctor.surename}</option>
                                    ))
                                }
                            </select>
                        </div>
                        <div className='mt-5 pt-3 col-12 col-md-6 d-flex flex-column align-items-start'>
                            ...
                            <select id='inputPatient' className="form-select"> {
                                patients.map((patient, index) => (
                                    <option key={index} value={patient.Id}>{patient.name + " " + patient.surename}</option>
                                ))
                            }
                            </select>
                        </div>
